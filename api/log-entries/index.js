...
var apiLogEntriesHandler = async (req, res) => {
  ...
  try {
    if (req.method === "GET") {
      ...
      await base(tableName).select({ view: "Grid view" }).eachPage(
      ...
      const mappedRecords = records.map((record) => {
        ...
        for (const [internalKey, airtableField] of Object.entries(fieldMap)) {
          const key = airtableField;
          mapped[internalKey] = fields[key] !== void 0 ? fields[key] : null;
        }
        return mapped;
      });
...
var log_entries_default = apiLogEntriesHandler;
export {
  log_entries_default as default
};
