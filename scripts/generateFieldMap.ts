...
async function main() {
  ...
  for (const table of data.tables || []) {
    ...
    fs.writeFileSync(
      path.join(schemaDir, `${toCamelCase(table.name)}.schema.json`),
  ...
  lines.push('}');
  lines.push('');
  lines.push('function filterMappedFields(data: Record<string, any>, fieldMap: Record<string, string>): Record<string, any> {');
  ...
  lines.push('}');
  lines.push('');
  lines.push('module.exports = { getFieldMap, filterMappedFields };');
