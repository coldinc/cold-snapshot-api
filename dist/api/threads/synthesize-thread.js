...
// lib/airtableBase.ts
var require_airtableBase = __commonJS({
  "lib/airtableBase.ts"(exports2, module2) {
    "use strict";
    ...
    module2.exports = getAirtableContext;
  }
});
...
var require_synthesisUtils = __commonJS({
  "lib/synthesisUtils.ts"(exports2, module2) {
    ...
    async function synthesizeThreadNarrative(threadId) {
      ...
      const logFieldMap = await getFieldMap2(TABLES.LOGS);
      const logRecords = await base(TABLES.LOGS).select({
      ...
      const logContentArray = logRecords.map((record) => record.fields?.[logFieldMap["Content"]]).filter(Boolean);
      ...
      const messages = [
        {
          role: "system",
          content: "You are an insightful assistant helping synthesize threads of work based on project logs. Provide a narrative summary and recommendations."
        },
        {
          role: "user",
...
// api/threads/synthesize-thread.ts
var synthesize_thread_exports = {};
__export(synthesize_thread_exports, {
  default: () => apiSynthesizeThreadHandler
});
module.exports = __toCommonJS(synthesize_thread_exports);
async function apiSynthesizeThreadHandler(req, res) {
  if (req.method !== "POST") {
...
//# sourceMappingURL=synthesize-thread.js.map